<?php
$helperPath = dirname(__DIR__) . '/helpers/maintenanceHelper.php';

if (file_exists($helperPath)) {
    require_once $helperPath;

    $isBlocked = isMaintenanceModeActive();

    $maintenanceInfo = $isBlocked ? getMaintenanceInfo() : null;

    if ($isBlocked) {
        $usuarioID = isset($_SESSION['id']) ? $_SESSION['id'] : null;
        $reason = $maintenanceInfo['reason'] ?? 'unknown';
        logBlockedAccess($usuarioID, $reason, 'requestProducts');
    }
} else {
    $isBlocked = false;
    ...
    <div class="maintenance-overlay">
        <div class="maintenance-modal">
            <div class="maintenance-logo-container">
                <img src="<?php echo htmlspecialchars($maintenanceInfo['logo']); ?>" alt="Kalli Logo" class="maintenance-logo">
                <div class="maintenance-logo-glow"></div>
            </div>
            ...
            <div class="maintenance-content">
                ...
                <h1 class="maintenance-title"><?php echo htmlspecialchars($maintenanceInfo['message']); ?></h1>
                <p class="maintenance-subtitle"><?php echo htmlspecialchars($maintenanceInfo['submessage']); ?></p>
                    ...
                    <div class="info-item">
                        ...
                        <div>
                            ...
                            <span class="info-value"><?php
                                                        ...
                                                        echo $dias[(int)$now->format('w')] . ', ' . $now->format('d/m/Y');
                                                        ?></span>
                        </div>
                    </div>
                ...
                <?php if ($maintenanceInfo['show_reasons'] && !empty($maintenanceInfo['reason_details'])): ?>
                    <div class="maintenance-reasons">
                        <h3><i class="fas fa-info-circle"></i> Motivo del bloqueo:</h3>
                        <ul>
                            <li><?php echo htmlspecialchars($maintenanceInfo['reason_details']); ?></li>
                        </ul>
                    </div>
                <?php endif; ?>
                ...
                <div class="maintenance-footer">
                    ...
                    <p class="footer-copyright">
                        © <?php echo date('Y'); ?> Kalli System - Todos los derechos reservados
                    </p>
                </div>
            </div>
        </div>
...
<?php else: ?>
    ...
    </div> <!-- Cierre de request-products-container -->
<?php endif; ?>