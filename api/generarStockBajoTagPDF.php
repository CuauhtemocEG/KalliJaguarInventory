...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(0, 10, utf8_decode('Productos con Inventario Bajo por Tag'), 0, 1, 'C');
        $this->Ln(5);
    }
...
try {
    ...
    $stmt = $conn->query("SELECT Nombre, Cantidad, Tipo, Tag FROM Productos WHERE Cantidad < 5 ORDER BY Tag DESC, Nombre");
...
header('Content-Disposition: attachment; filename="stock_bajo_por_tag.pdf"');
$pdf->Output('D', 'stock_bajo_por_tag.pdf');
exit;
